...
Cache *initCache()
{
    ...
    //FOR SHIP SIGNATURE HISTORY COUNTER TABLE
...
//    printf("Num of blocks: %u\n", cache->num_blocks);
	...
	//FOR SHIP REUSE BIT AND SIGNATURE BITS
	cache->blocks[i].reuse = false;
...
bool accessBlock(Cache *cache, Request *req, uint64_t access_time)
{
	...
	//printf("%lu\n", req->PC);
	// FOR SHIP
	//cache->signature = req->PC & TABLE_SIZE;
	//req->PC & TABLE_SIZE ==== SIGNATURE
	if (cache->table[blk->signature] < 4)
	{
		cache->table[blk->signature] += 1;
	}
	// BLOCK FOUND IN CACHE, SET REUSE TO TRUE/1
	blk->reuse = true;
...
bool insertBlock(Cache *cache, Request *req, uint64_t access_time, uint64_t *wb_addr)
{
    ...
    //FOR SHIP
    if (victim->reuse != true)
    {
	//if (cac\he->table[victim->signature] > 0)
		cache->table[victim->signature] -= 1;
    }
    else
    {
	victim->reuse = false;
    ...
    //FOR SHIP
    victim->signature = req->PC & TABLE_SIZE;
    //printf("%lu\n", req->PC);
    //printf("%u\n", victim->signature);
    ...
    //++victim->frequency;
    if (cache->table[victim->signature] > 0)
    {
	victim->when_touched = access_time;
    }
