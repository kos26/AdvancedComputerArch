...
void tick(Controller *controller)
{
	...
	{
	    controller->conflicts += 1;
	}
	#endif

	#ifdef OOO
	Node *ooo_first = controller->waiting_queue->first;
	while (ooo_first)
	{

		int target_bank_id = ooo_first->bank_id;

		if ((controller->bank_status)[target_bank_id].next_free <= controller->cur_clk)
		{
		    ooo_first->begin_exe = controller->cur_clk;
		    if (ooo_first->req_type == READ)
		    {
		        ooo_first->end_exe = ooo_first->begin_exe + (uint64_t)nclks_read;
		    }
		    else if (ooo_first->req_type == WRITE)
		    {
		        ooo_first->end_exe = ooo_first->begin_exe + (uint64_t)nclks_write;
		    }
		    // The target bank is no longer free until this request completes.
		    (controller->bank_status)[target_bank_id].next_free = ooo_first->end_exe;

		    migrateToQueue(controller->pending_queue, ooo_first);
		    deleteNode(controller->waiting_queue, ooo_first);
		    break;
		}

		if (ooo_first == controller->waiting_queue->last)
		{
		    break;
		}
		
		ooo_first = ooo_first->next;
